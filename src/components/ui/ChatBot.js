  ...
  return (
    <div className="fixed bottom-28 right-8 z-[150] flex flex-col items-end pointer-events-none">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="w-[320px] md:w-[380px] h-[600px] glass-frosted rounded-[4rem] border-white/10 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.6)] mb-8 flex flex-col overflow-hidden pointer-events-auto relative"
          >
            {/* Cinematic Background Lines */}
            <div className="absolute inset-0 bg-blueprint-fine opacity-5 pointer-events-none" />

            <div className="bg-white/5 border-b border-white/10 p-8 flex items-center justify-between relative z-10">
              <div className="flex items-center space-x-4 text-white">
                ...
                <div>
                  <h3 className="text-xs font-black uppercase tracking-[0.4em] leading-none mb-2 text-white">KCS Intelligence</h3>
                  <div className="flex items-center space-x-2">
                    <div className="w-1.5 h-1.5 bg-accent rounded-full animate-pulse shadow-[0_0_10px_#FBBF24]" />
                    <span className="text-[7px] font-black uppercase tracking-widest text-white/40">Nexus Local Sync v3.1</span>
                  </div>
                </div>
              </div>
              <button 
                onClick={() => setIsOpen(false)} 
                className="w-10 h-10 glass-frosted border-white/5 rounded-full flex items-center justify-center text-white/30 hover:text-white hover:border-white/20 transition-all hover:scale-110"
              >
                <ChevronDown size={20} />
              </button>
            </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-hide relative z-10">
              {history.map((chat, idx) => (
                ...
                >
                  <div className={`max-w-[90%] p-6 rounded-[2.5rem] shadow-xl ${chat.type === "bot" ? "glass-frosted border-white/5" : "bg-accent/10 border border-accent/20"}`}>
                    <p className={`text-[11px] font-black tracking-tight leading-relaxed uppercase italic ${chat.type === "bot" ? "text-white/80" : "text-accent"}`}>
                      {chat.message}
                    </p>
                      ...
                      <div className="mt-6 flex flex-col space-y-3">
                        {chat.options.map((opt, i) => (
                          <motion.button 
                            ...
                            className="text-left text-white/40 text-[9px] font-black uppercase tracking-[0.2em] px-6 py-4 glass-frosted border-white/5 rounded-full transition-all flex items-center justify-between group"
                          >
                            <span>{opt.label}</span>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="glass-frosted border-white/5 px-6 py-3 flex space-x-2 rounded-full">
            ...
            <form onSubmit={handleSubmit} className="p-8 pb-10 bg-black/40 border-t border-white/10 group relative z-10">
               <div className="relative flex items-center">
                  <input
                    ...
                    className="w-full glass-frosted border-white/10 p-5 rounded-full text-[10px] text-white focus:outline-none focus:border-accent/50 transition-all font-black uppercase tracking-widest placeholder:text-white/20 pr-14 shadow-inner"
                  />
                  <button type="submit" className="absolute right-3 w-10 h-10 glass-frosted border-white/10 rounded-full flex items-center justify-center text-accent hover:scale-110 active:scale-90 transition-all shadow-xl">
                    <Send size={16} />
                  </button>
               </div>
               <div className="mt-5 flex items-center justify-between px-2">
                  <span className="text-[6px] font-black uppercase tracking-[0.5em] text-white/15">Neural Sync Status: SECURE</span>
      ...
      <motion.button
        ...
        className="glass-frosted border-white/10 text-accent w-16 h-16 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-center justify-center relative group pointer-events-auto rounded-3xl"
      >
        {/* Kinetic Ring */}
        ...
        {!isOpen && (
          <span className="absolute -top-1 -right-1 flex h-4 w-4">
            ...
            <span className="relative inline-flex rounded-full h-4 w-4 bg-accent shadow-[0_0_15px_#FBBF24] border border-white/20"></span>
          </span>
        )}
      </motion.button>
    </div>
